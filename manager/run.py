...
def run_package(current_path, args = None, binary_args = None, environment_variables = None, perf = True, dry_run = False):
    run_command = (lambda command: print(f"Running: {command}")) if dry_run else os.system
    ...
    is_error_free = True if {"--disable-check", "--no-check"}.intersection(args) else not run_command("cargo check --no-default-features --features debug")
    if not is_error_free:
        return
    is_release = "--debug" not in args
    if not build_binary(is_release):
        executable = "timecat_debug.exe" if sys.platform == "win32" else "timecat_debug"
        ...
        if perf and which("perf") is not None:
            need_to_run_without_perf = bool(timed_run(lambda: run_command(f"perf record {executable_run_command}")))
